...
// Search following users from a user
func SearchFollowing(w http.ResponseWriter, r *http.Request) {
	...
	repository := repositories.NewRepositoryUser(db)
	following, err := repository.SearchFollowingFromUser(userId)
	if err != nil {
		responses.Error(w, http.StatusBadRequest, err)
		return
	}
...
func UpdatePassword(w http.ResponseWriter, r *http.Request) {
	...
	repository := repositories.NewRepositoryUser(db)
	hashedPassword, err := repository.GetPasswordByUserId(userId)
	if err != nil {
		responses.Error(w, http.StatusInternalServerError, errors.New("password invalid"))
		return
	}

	if err := security.CheckHashPassword(password.Current, hashedPassword); err != nil {
		responses.Error(w, http.StatusUnauthorized, err)
		return
	}
